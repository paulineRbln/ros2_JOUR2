/**
 * Exercise 9: Constrained pick-and-place (copy of exercise 8) with orientation constraints
 * Plan motion with orientation constraints (e.g., keep gripper oriented downward during motion)
...
static const std::string LOGNAME = "exercise_09_constrained_motion";
...
// TODO : Create an AttachedCollisionObject message to detach the object, fill it and apply it to the planning scene
   ...
   // TODO : Re-add the object to the planning scene at the place location
}

// Helper: Plan and execute a motion
bool planAndExecute(
...

{
    ...
    RCLCPP_INFO(logger, "Starting Exercise 9: Constrained Pick-and-Place with Franka FR3...");
    ...
    addObstaclesAndObject(planning_scene_interface, moveit_cpp_ptr, graspable_object, cylinder_pose, cylinder);
    rclcpp::sleep_for(std::chrono::seconds(2));

    // --- Add position constraint: limit maximum Z height ---
    // TODO : Define position constraint to limit maximum Z height of end-effector
    
    // Define a box constraint for the allowed region
    // TODO : Define the box shape (dimensions)
    
    // Position the constraint box so its top is at max allowed Z
    // TODO : Define the box pose (position and orientation)

    // Set the path constraint for subsequent planning steps
    // TODO : Set the path constraints in the planning component
